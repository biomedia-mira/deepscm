...
class TestNamedMultivariate(unittest.TestCase):
    ...
    def setUp(self):
        factors = [td.MultivariateNormal(torch.zeros(d), torch.eye(d)) for d in self.var_shapes]
        self.dist = Factorised(factors)
        self.assertEqual(self.dist.event_shape, (self.total_ndims,))
        self.named = Factorised(factors, var_names=self.var_names)
    ...
    def test_constructor(self):
        with self.assertRaises(ValueError):
            Factorised(self.dist.factors, var_names=self.var_names[:-1])
    ...
    def test_marginalise_multi(self):
        for names in [['a'], ['a', 'b'], ['b', 'a']]:
            ...
            self.assertSequenceEqual(marg.variable_names, names)
    ...
    def test_condition(self):
        for names in [['a'], ['a', 'b'], ['b', 'a']]:
            ...
            self.assertSequenceEqual(cond.variable_names, marg_names)
